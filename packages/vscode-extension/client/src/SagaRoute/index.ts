...
function getSagarouteClass(): typeof BundledSagaRoute {
  ...
  try {
    if (!vscode.workspace.isTrusted) {
      logging.logMessage(
        "This workspace is not trusted. Using the bundled sagaroute."
      );
      return BundledSagaRoute;
    }
    const stats = fs.statSync(
      path.join(workspaceRootFolderPath, "package.json")
    );
    if (stats?.isFile()) {
      ...
      logging.logMessage(
        "The file [package.json] is not exist. Using the bundled sagaroute."
      );
      return BundledSagaRoute;
    }
  } catch (err) {
